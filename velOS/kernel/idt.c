 ...
 *
 * Role: The IDT tells the CPU where to jump when an interrupt or exception
 * occurs. We map all 32 CPU exceptions and the 16 hardware IRQs to our assembly
...
/* External assembly stubs from isr.asm */
...
/**
 * idt_set_gate - Fills one entry in the IDT
 */
...
/**
 * idt_init - Zeroes the IDT, remaps the PIC, sets the gates, and loads the IDT
 */
void idt_init(void) {
    /* 1. Zero out all entries */
    for (int i = 0; i < 256; i++) {
        idt_set_gate(i, 0);
    }

    /* 2. Call pic_init to remap the PIC */
    pic_init();

    /* 3. Set gates for CPU Exceptions (ISR 0-31) */
    ...
    /* 4. Set gates for Hardware IRQs (IRQ 0-15 mapped to ISR 32-47) */
